...
1. For a maintenance pass, discover failed workflows, actionable open Issues,
   and approved unfinished work. Deduplicate candidates before acting. Report a
   verified no-op only when evidence is sufficient and no candidate needs work;
...
6. Implement the smallest complete change inside the authorized scope.
7. Add or update the smallest durable check for the defect or acceptance criteria.
8. Run focused checks, then repository-required broader checks.
9. Default to one independently reviewable outcome. Continue to another
   candidate only when the caller's explicit scope or budget includes it.
10. Deliver through the permitted Git and PR workflow.
